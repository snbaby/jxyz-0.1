...
const currDay = moment().format('yyyy-MM-DD')
  ...
  //处理部门数据
  function getTreeData(source,parentId ) {
...
async function main(deptCode,total,key,extendList,toFix,text=''){
    // 查询组织信息
    let sql = `select code,name,level,parent_code as parentCode from t_grid_m where all_parent_code like '%${deptCode}%'`
    extendList.forEach(item =>{
        sql = sql + ` and all_parent_code not like '%${item}%'`
    })
...
function insertData(list,total,key,toFix,text){
    if (list) {
        if (list.length == 1) {
            const item = list[0]
            const value = parseFloat(total).toFixed(toFix) + text
            ...
            const offset = (baseNum/10).toFixed(0) // 获取基数
            const first = list.pop()
            for(let index in list){
                const item = list[index]
                const randowNum = Math.random()*offset
                if(index % 2 == 1){
                    item.value = parseFloat(randowNum + baseNum).toFixed(toFix) + text
                } else {
                    item.value = parseFloat(baseNum-randowNum).toFixed(toFix) + text
                }
            ...
            if(first){
              first.value = parseFloat(total-totalNum).toFixed(toFix) + text
...
getData()
function getData(){
  let sql = 'select a.`key`,a.`value`,a.grid_code,b.name from t_grid_statistics a LEFT JOIN t_layer_m b on b.code = a.key where level = 4'
  pool.query(sql).then(res =>{
    const arr1 = res.filter(item => item.name.indexOf('收入') > -1)
    const arr2 = res.filter(item => item.key == 'eFKz3RDb' ) // 当月环比增幅
    const arr4 = res.filter(item => item.key == 'pcNXGE7K' ) // 昨日环比增幅
